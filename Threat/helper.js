({
    ...
    action.setCallback(this, function (response) {
      ...
      if (state === "SUCCESS") {
        if (response.getReturnValue().isSuccess === true) {
          ...
          this.singleBarChart(component, event, helper, topApplicationchartDiv, "Top Application", "#A0AAAD");
    ...
    var plot = new Highcharts.Chart(divName, {
      ...
      legend: {
        ...
        labelFormatter: function () {
          return `<span style="min-width: 150px; display:inline-block;">
                                <span title="${
                                  this.name
                                }" style="float:left;width: 70px;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;">
                                    ${this.name}
                                </span>
      ...
      for (var v in dataToPlot) {
        if (dataToPlot[v].date == listOfDate[ld]) {
          ...
          if (existingThreatIndex !== -1) {
            ...
            shaList.push({
              threatName: dataToPlot[v].threatName,
              count: dataToPlot[v].hashes_by_date,
              link: dataToPlot[v].hash_link,
            });
          }
        }
      }
        ...
        labels: {
          ...
          formatter: function () {
            ...
            if (link) {
              ...
              return (
                '<div class="bar-label hashe-label" title="' +
                truncatedShaName +
  ...
  downloadToCSV: function (component, event, helper, csv, sectionName) {
    var fileName =
      "Threat Report - " +
      component.get("v.headerDetail").Account_Name +
      " - " +
